...
from .agent import DaytonaSandboxAgent
...
class AgentTaskManager(InMemoryTaskManager):
    ...
    def __init__(self, agent: DaytonaSandboxAgent):
        super().__init__()
        self.agent = agent
    ...
    def _validate_request(
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, DaytonaSandboxAgent.SUPPORTED_CONTENT_TYPES
        ):
            logger.warning(
                "Unsupported output mode. Received %s, Support %s",
                task_send_params.acceptedOutputModes,
                DaytonaSandboxAgent.SUPPORTED_CONTENT_TYPES,
            )
